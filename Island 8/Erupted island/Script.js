...
dragons.forEach(color => {
    ...
    database.ref(`dragons/${color}`).on('value', (snap) => {
        ...
        const data = snap.val() || {};
        // CRITICAL: Listen for Island 8 (the base island ID for Volcano)
        if (data.island == 8) {
...
const relic1 = document.getElementById('relic1'); // Ice Pack
const relic2 = document.getElementById('relic2'); // Red Amulet
...
islandRelicsRef.on('value', (snapshot) => {
    ...
    if(relic1) relic1.style.display = takenData.icePack ? 'none' : 'block';
    if(relic2) relic2.style.display = takenData.redDragonAmulet ? 'none' : 'block';
});
...
function checkRelics(playerRect) {
    if (relic1 && relic1.style.display !== 'none' && areRectsOverlapping(playerRect, relic1.getBoundingClientRect())) {
         ...
         myPlayerRef.child('relics/icePack').set(true);
         islandRelicsRef.child('icePack').set(true);
    }
    if (relic2 && relic2.style.display !== 'none' && areRectsOverlapping(playerRect, relic2.getBoundingClientRect())) {
         ...
         myPlayerRef.child('relics/redDragonAmulet').set(true);
         islandRelicsRef.child('redDragonAmulet').set(true);
    }
}
