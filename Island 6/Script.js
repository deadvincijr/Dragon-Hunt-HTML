...
function calculateDragonDamage(color) {
    ...
    else if (myIslandId === 4 && amIHiding && playerCountOnMyIsland > 1) msg += " (Hiding failed: Too many players!)";
    return { dmg, msg };
}
...
const relic1 = document.getElementById('relic1'); // Water Bucket
const relic2 = document.getElementById('relic2'); // Hazmat
const relic3 = document.getElementById('relic3'); // Great Sword
const relic4 = document.getElementById('relic4'); // Dragon Staff
const relic5 = document.getElementById('relic5'); // Weather Staff
...
islandRelicsRef.on('value', (snapshot) => {
    ...
    if(relic1) relic1.style.display = takenData.waterBucket ? 'none' : 'block';
    if(relic2) relic2.style.display = takenData.hazmatSuit ? 'none' : 'block';
    if(relic3) relic3.style.display = takenData.greatSword ? 'none' : 'block';
    if(relic4) relic4.style.display = takenData.dragonStaff ? 'none' : 'block';
    if(relic5) relic5.style.display = takenData.weatherStaff ? 'none' : 'block';
});
...
function checkRelics(playerRect) {
    if (relic1 && relic1.style.display !== 'none' && areRectsOverlapping(playerRect, relic1.getBoundingClientRect())) {
         alert("You found the Water Bucket!");
         myPlayerRef.child('relics/waterBucket').set(true);
         islandRelicsRef.child('waterBucket').set(true);
    }
    if (relic2 && relic2.style.display !== 'none' && areRectsOverlapping(playerRect, relic2.getBoundingClientRect())) {
         alert("You found the Hazmat Suit!");
         myPlayerRef.child('relics/hazmatSuit').set(true);
         islandRelicsRef.child('hazmatSuit').set(true);
    }
    if (relic3 && relic3.style.display !== 'none' && areRectsOverlapping(playerRect, relic3.getBoundingClientRect())) {
         alert("You found the Great Sword!");
         myPlayerRef.child('relics/greatSword').set(true);
         islandRelicsRef.child('greatSword').set(true);
    }
    if (relic4 && relic4.style.display !== 'none' && areRectsOverlapping(playerRect, relic4.getBoundingClientRect())) {
         alert("You found the Dragon Staff!");
         myPlayerRef.child('relics/dragonStaff').set(true);
         islandRelicsRef.child('dragonStaff').set(true);
    }
    if (relic5 && relic5.style.display !== 'none' && areRectsOverlapping(playerRect, relic5.getBoundingClientRect())) {
         alert("You found the Weather Staff!");
         myPlayerRef.child('relics/weatherStaff').set(true);
         islandRelicsRef.child('weatherStaff').set(true);
    }
}
...
function gameLoop() {
    ...
    const playerRect = myPlayerDiv.getBoundingClientRect();
    let hasMoved = false;
    const obstaclesRects = Array.from(obstacles).map(o => o.getBoundingClientRect());
    ...
    if (keysPressed['d']) {
        ...
        let canMove = true; for (const r of obstaclesRects) { if (areRectsOverlapping(futureRect, r)) { canMove = false; break; } }
...
allPlayersRef.on('value', (snapshot) => {
    ...
    allPlayerIdsOnPage.forEach(colorId => { document.getElementById(colorId).style.display = 'none'; });
