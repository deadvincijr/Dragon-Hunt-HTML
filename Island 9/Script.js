...
const relic1 = document.getElementById('relic1'); // Spyglass
const sailboat = document.getElementById('sailboat'); 
const weatherStaff = document.getElementById('weatherstaff');
...
islandRelicsRef.on('value', (snapshot) => {
    ...
    if(relic1) relic1.style.display = takenData.spyglass ? 'none' : 'block';
    if(sailboat) sailboat.style.display = takenData.sailboat ? 'none' : 'block';
    if(weatherStaff) weatherStaff.style.display = takenData.weatherStaff ? 'none' : 'block';
});
...
function checkRelics(playerRect) {
    if (relic1 && relic1.style.display !== 'none' && areRectsOverlapping(playerRect, relic1.getBoundingClientRect())) {
         alert("You found a Spyglass!");
         myPlayerRef.child('relics/spyglass').set(true);
         islandRelicsRef.child('spyglass').set(true);
    }
    if (sailboat && sailboat.style.display !== 'none' && areRectsOverlapping(playerRect, sailboat.getBoundingClientRect())) {
         alert("You found a Sailboat!");
         myPlayerRef.child('relics/sailboat').set(true);
         islandRelicsRef.child('sailboat').set(true);
    }
    if (weatherStaff && weatherStaff.style.display !== 'none' && areRectsOverlapping(playerRect, weatherStaff.getBoundingClientRect())) {
