...
const binoculars = document.getElementById('binoculars'); // Green Amulet
...
islandRelicsRef.on('value', (snapshot) => {
    ...
    if (binoculars) binoculars.style.display = takenData.greenDragonAmulet ? 'none' : 'block';
});
...
function checkRelics(playerRect) {
    if (binoculars && binoculars.style.display !== 'none' && areRectsOverlapping(playerRect, binoculars.getBoundingClientRect())) {
         alert("Congrats! You found the Green Dragon Amulet!");
         myPlayerRef.child('relics/greenDragonAmulet').set(true);
         islandRelicsRef.child('greenDragonAmulet').set(true);
    }
}
