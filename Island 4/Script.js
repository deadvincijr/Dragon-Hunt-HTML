...
function gameLoop() {
    ...
    if (keysPressed['d']) {
        ...
        if (currentX + MOVEMENT_SPEED <= 480 && canMove) { currentX += MOVEMENT_SPEED; hasMoved = true; } // Boundary 480
    }
    if (keysPressed['a']) {
        const futureRect = { top: playerRect.top, bottom: playerRect.bottom, left: playerRect.left - MOVEMENT_SPEED, right: playerRect.right - MOVEMENT_SPEED };
        let canMove = true; for (const r of obstaclesRects) { if (areRectsOverlapping(futureRect, r)) { canMove = false; break; } }
    ...
    if (keysPressed['w']) {
        const futureRect = { top: playerRect.top - MOVEMENT_SPEED, bottom: playerRect.bottom - MOVEMENT_SPEED, left: playerRect.left, right: playerRect.right };
        let canMove = true; for (const r of obstaclesRects) { if (areRectsOverlapping(futureRect, r)) { canMove = false; break; } }
        if (currentY + MOVEMENT_SPEED <= 490 && canMove) { currentY += MOVEMENT_SPEED; hasMoved = true; } // Boundary 490
    }
