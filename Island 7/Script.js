...
database.ref('game_state/current_round').on('value', snap => {
    currentRound = parseInt(snap.val()) || 1;
});
...
function queueDragonEvent(damage, message) {
    ...
    eventBatchTimer = setTimeout(executeDragonBatch, 300);
}
...
globalTeleportRef.on('value', (snapshot) => {
    ...
    if (data && data.url) {
        ...
        let newUrl = data.url;
        if (newUrl.includes('?')) newUrl += `&player=${myColor}`; else newUrl += `?player=${myColor}`;
...
const relic1 = document.getElementById('relic1'); // Blue Amulet
const relic2 = document.getElementById('relic2'); // Hazmat
const wizardStaff = document.getElementById('wizardstaff'); 
...
islandRelicsRef.on('value', (snapshot) => {
    ...
    if(relic1) relic1.style.display = takenData.blueDragonAmulet ? 'none' : 'block';
    if(relic2) relic2.style.display = takenData.hazmatSuit ? 'none' : 'block';
    if(wizardStaff) wizardStaff.style.display = takenData.wizardstaff ? 'none' : 'block';
});
...
if (amuletSubmitBtn) {
    amuletSubmitBtn.addEventListener('click', () => {
        ...
        if (activeEntity) {
            myPlayerRef.child('amuletSelection').set({ dragon: activeEntity, island: selectedIsland, round: currentRound }).then(() => {
...
function checkRelics(playerRect) {
    if (relic1 && relic1.style.display !== 'none' && areRectsOverlapping(playerRect, relic1.getBoundingClientRect())) {
         alert("You found the Blue Dragon Amulet!");
         myPlayerRef.child('relics/blueDragonAmulet').set(true);
         islandRelicsRef.child('blueDragonAmulet').set(true);
    }
    ...
    if (wizardStaff && wizardStaff.style.display !== 'none' && areRectsOverlapping(playerRect, wizardStaff.getBoundingClientRect())) {
         alert("You found the Wizard's Staff!");
         myPlayerRef.child('relics/wizardstaff').set(true);
         islandRelicsRef.child('wizardstaff').set(true);
    }
}
